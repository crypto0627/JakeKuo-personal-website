...
export function CyberAvatar({ src, alt, size = "md", id, className, progressValue = 75 }: CyberAvatarProps) {
  const sizeClasses = {
    sm: "w-24 h-24",
          ...
          <div className="w-full h-full relative">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-2">
              <div className="h-1 w-full bg-primary/50 rounded-full overflow-hidden">
                <div className="h-full bg-primary animate-pulse" style={{ width: `${progressValue}%` }}></div>
              </div>
            </div>
          </div>
        </div>
